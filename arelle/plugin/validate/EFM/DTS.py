    ...
    for referencedDocument, modelDocumentReference in modelDocument.referencesDocument.items():
        ...
        if modelDocumentReference.referenceType == "include":
            val.modelXbrl.error(("EFM.6.07.01", "GFM.1.03.01"),
                _("[du-0701-Schema-Include-Present] The schema contains an 'include' element, in $(refSource).  Please remove it and resubmit."),
        ...
        if modelDocument.uri in val.disclosureSystem.standardTaxonomiesDict:
            ...
            if len(modelDocument.basename) > 32:
                val.modelXbrl.error("EFM.5.01.01.tooManyCharacters",
                    _("Document file name $(filename) must not exceed 32 characters."),
            ...
            if modelDocument.type == ModelDocument.Type.INLINEXBRL:
                if not htmlFileNamePattern.match(modelDocument.basename):
                    val.modelXbrl.error("EFM.5.01.01",
                        _("Document file name $(filename) must start with a-z or 0-9, contain lower case letters, ., -, _, and end with .xsd or .xml."),
                        modelObject=modelDocument, filename=modelDocument.basename)
    ...
    if (modelDocument.type == ModelDocument.Type.SCHEMA and 
        ...
        if targetNamespaceAuthority in val.disclosureSystem.standardAuthorities:
            val.modelXbrl.error(("EFM.6.07.03", "GFM.1.03.03"),
                _("[fs-0703-Extension-Has-Standard-Namespace-Authority]  The target namespace, $(targetNamespace) cannot have the same authority ($(targetNamespaceAuthority)) as a standard taxonomy, in $(schema).  Please change your target namespace."),
        ...
        if match is None:
            val.modelXbrl.error(("EFM.6.07.04", "GFM.1.03.04"),
                _("[cp-0704-Taxonomy-Valid-Target-Namespace] You did not adhere to the requirements for target namespace, for $(targetNamespace) in $(schema).  Please recheck your submission and adhere to the target namespace requirements."),
                modelObject=modelDocument, schema=os.path.basename(modelDocument.uri), targetNamespace=modelDocument.targetNamespace)
        ...
        if modelDocument.targetNamespace is not None:
            ...
            if not re.match(r"(http://|https://|ftp://|urn:)\w+",authority):
                val.modelXbrl.error(("EFM.6.07.05", "GFM.1.03.05"),
                    _("Taxonomy schema $(schema) namespace $(targetNamespace) must be a valid URI with a valid authority for the namespace."),
                    modelObject=modelDocument, schema=os.path.basename(modelDocument.uri), targetNamespace=modelDocument.targetNamespace)
            ...
            if not prefixes:
                ...
                val.modelXbrl.error(("EFM.6.07.07", "GFM.1.03.07"),
                    _("[du-0707-Recommended-Prefix-Disallowed]  The schema does not supply a prefix for $(targetNamespace) without an underscore character, in file $(schema). Please provide or change a prefix"),
                    modelObject=modelDocument, schema=os.path.basename(modelDocument.uri), targetNamespace=modelDocument.targetNamespace)
            else:
                prefix = prefixes[0]
                if len(prefixes) > 1:
                ...
                if isinstance(modelConcept,ModelConcept):
                        ...
                        if c.modelDocument != modelDocument:
                            if not c.modelDocument.uri.startswith(val.modelXbrl.uriDir):
                                val.modelXbrl.error(("EFM.6.07.16", "GFM.1.03.18"),
                                    _("[cp-0716-Element-Name-Same-As-Base] Your extension taxonomy contains an element, $(concept), which has the same name as an element in the base taxonomy, $(standardConcept).  Please ensure that this extension is appropriate and if so, please change the extension concept name."),
                    ...
                    if _id != requiredId:
                        val.modelXbrl.error(("EFM.6.07.17", "GFM.1.03.19"),
                            _("[cp-0717-Element-Id]  You did not adhere to the declarations for concepts by containing an 'id' attribute whose value begins with the recommended namespace prefix of the taxonomy, followed by an underscore, followed by an element name, for the concept $(concept).  Please recheck your submission."),
                    ...
                    if nillable != "true" and modelConcept.isItem:
                        val.modelXbrl.error(("EFM.6.07.18", "GFM.1.03.20"),
                            _("[du-0718-Nillable-Not-True]  Element $(concept) is declared without a 'true' value for the nillable attribute, in $(refSource).  Please set the value to 'true'."),
                    ...
                    if modelConcept.isTypedDimension:
                        val.modelXbrl.error(("EFM.6.07.20", "GFM.1.03.22"),
                            _("[du-0720-Typed-Domain-Ref-Disallowed]  There is an xbrldt:typedDomainRef attribute on $(concept), in $(refSource). Please remove it."),
                            modelObject=modelConcept, concept=modelConcept.qname,
                    ...
                    if modelConcept.isAbstract and not isDuration:
                        val.modelXbrl.error(("EFM.6.07.21", "GFM.1.03.23"),
                            _("[du-0721-Abstract-Is-Instant]  Element $(concept) is declared as an abstract item with period type 'instant', in $(refSource).  Please change it to 'duration' or make the element not abstract."),
                    ...
                    if name.endswith("Axis") ^ (substitutionGroupQname == XbrlConst.qnXbrldtDimensionItem):
                        val.modelXbrl.error(("EFM.6.07.23", "GFM.1.03.25"),
                            _("[du-0723-Axis-Dimension-Name-Mismatch] The substitution group 'xbrldt:dimensionItem' is only consistent with an element name that ends with 'Axis'.  Please change $local-name($(concept)) or change the substitutionGroup, in $(refSource)."),
                            modelObject=modelConcept, concept=modelConcept.qname)
                    ...
                    if name.endswith("Table") ^ (substitutionGroupQname == XbrlConst.qnXbrldtHypercubeItem):
                        val.modelXbrl.error(("EFM.6.07.24", "GFM.1.03.26"),
                            _("[du-0724-Table-Hypercube-Name-Mismatch] The substitution group 'xbrldt:hypercubeItem' is only allowed with an element name that ends with 'Table'.  Please change $local-name($(concept)) or change the substitutionGroup, in $(refSource)."),
                            modelObject=modelConcept, schema=os.path.basename(modelDocument.uri), concept=modelConcept.qname)
                    ...
                    if substitutionGroupQname not in (None,
                        ...
                        val.modelXbrl.error(("EFM.6.07.25", "GFM.1.03.27"),
                            _("[du-0725-Substitution-Group-Custom] The substitution group attribute value $(substitutionGroup) of element $local-name($(concept)) is not allowed, in $(refSource).  Please change it to one of 'xbrli:item', 'xbrldt:dimensionItem' or 'xbrldt:hypercubeItem'."),
                            modelObject=modelConcept, concept=modelConcept.qname,
                    ...
                    if name.endswith("LineItems") and modelConcept.abstract != "true":
                        val.modelXbrl.error(("EFM.6.07.26", "GFM.1.03.28"),
                            _("[du-0726-LineItems-Abstract-Name-Mismatch] The element $local-name($(concept)) ends with 'LineItems' but is not abstract. Please change $local-name($(concept)) or the value of the 'abstract' attribute, in $(refSource)."),
                            modelObject=modelConcept, concept=modelConcept.qname)
                    ...
                    if isDomainItemType != endsWithDomainOrMember:
                        val.modelXbrl.error(("EFM.6.07.27", "GFM.1.03.29"),
                            _("[du-0727-Domain-Type-Name-Mismatch] The type 'us-types:domainItemType' is only allowed with an element name that ends with 'Domain' or 'Member'.  Please change $local-name($(concept)) or change the type, in $(refSource)."),
                            modelObject=modelConcept, concept=modelConcept.qname)

                    # 6.7.28 domainItemType must be duration
                    if isDomainItemType and not isDuration:
                        val.modelXbrl.error(("EFM.6.07.28", "GFM.1.03.30"),
                            _("[du-0728-Domain-Member-Is-Instant] Element $local-name($(concept)) is declared as a us-types:domainItemType with period type 'instant', in $(refSource).  Please change it to 'duration' or change the item type."),
                            modelObject=modelConcept, concept=modelConcept.qname)
                                                
                    #6.7.31 (version 27) fractions
                    if modelConcept.isFraction:
                        val.modelXbrl.error("EFM.6.07.31",
                            _("[du-0731-Fraction-Item-Type] Element $(concept) is declared as a fraction item type.  Change or remove the declaration."),
                            modelObject=modelConcept, concept=modelConcept.qname)
                    ...
                    if modelConcept.isItem and (not modelConcept.isNumeric and not isDuration and not modelConcept.isAbstract and not isDomainItemType):
                        val.modelXbrl.error("EFM.6.07.32",
                            _("[rq-0732-Nonnnumeric-Must-Be-Duration] Declaration of element $(concept) in $(schema) must have xbrli:periodType of 'duration' because its base type is not numeric."),
                            modelObject=modelConcept, schema=os.path.basename(modelDocument.uri), concept=modelConcept.qname)
        ...
        for e in modelDocument.xmlRootElement.iterdescendants(tag="{http://www.xbrl.org/2003/linkbase}linkbase"):
            if isinstance(e,ModelObject):
                val.modelXbrl.error(("EFM.6.07.08", "GFM.1.03.08"),
                    _("[cp-0708-No-Embedded-Linkbases] Your filing contained embedded linkbases in $(schema).  Please recheck your submission and remove all embedded linkbases."),
            ...
            if isinstance(e,ModelObject):
                ...
                if targetNamespaceAuthority != UrlUtil.authority(roleURI):
                    val.modelXbrl.error(("EFM.6.07.09", "GFM.1.03.09"),
                        _("[du-0709-Role-Namespace-Mismatch] Role $(roleType) does not begin with $(targetNamespace)'s scheme and authority, in $(refSource). Please change the role URI or target namespace URI."),
                    ...
                    if len(modelRoleTypes) == 1:
                        ...
                        if not usedOns.isdisjoint(requiredUsedOns) and len(requiredUsedOns - usedOns) > 0:
                            val.modelXbrl.error(("EFM.6.07.11", "GFM.1.03.11"),
                                _("[du-0711-Role-Type-Declaration-Incomplete] The role $(roleType) did not provide a usedOn element for all three link types (presentation, calculation and definition), in $(refSource). Change the declaration to be for all three types of link, and resubmit."),
            ...
            if isinstance(e,ModelObject):
                ...
                if targetNamespaceAuthority != UrlUtil.authority(arcroleURI):
                    val.modelXbrl.error(("EFM.6.07.13", "GFM.1.03.15"),
                        _("[du-0713-Arcrole-Namespace-Mismatch]  Relationship role $(arcroleType) does not begin with $(targetNamespace)'s scheme and authority, in $(refSource).  Please change the relationship role URI or target namespace URI."),
                ...
                if definition is None or not arcroleDefinitionPattern.match(definition):
                    val.modelXbrl.error(("EFM.6.07.15", "GFM.1.03.17"),
                        _("[du-0715-Arcrole-Definition-Missing]  Relationship role declaration $(arcroleType) is missing a definition, in $(refSource).  Please provide a definition."),
                        modelObject=e, arcroleType=arcroleURI)
                            ...
                            for rel in labelRels.toModelObject(labelElt):
                                if rel.fromModelObject is not None and not rel.fromModelObject.isNumeric:
                                    val.modelXbrl.error("EFM.6.10.09",
                                        _("[du-1009-Numeric-Label-Role] Non-numeric element $(concept) has a label role for numeric elements: $(role). Please change the role attribute."), 
