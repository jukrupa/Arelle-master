            ...
            for hasHcRels in hasHypercubeRelationships.values():
                for hasHcRel in hasHcRels:
                    ...
                    if hasHypercubeArcrole == XbrlConst.all:
                        ...
                        if hasHcRel.isClosed:
                            val.modelXbrl.error(("EFM.6.16.06", "GFM.1.08.06"),
                                _("[fs-1606-Not-All-Relationship-Is-Closed] The notAll relationship for table $(hypercube) must have the closed attribute set to 'false' so that it can achieve its intended purpose of excluding combinations of domain values, on relationship from $(primaryItem), in $(refSource). Please recheck submission."),
                        ...
                        if hc in positiveHypercubes:
                            val.modelXbrl.error(("EFM.6.16.08", "GFM.1.08.08"),
                                _("[fs-1608-Table-Excludes-Itself] Element $(hypercube) is a table that negates itself in $(linkroleDefinition) for primary item $(primaryItem).  Modify the 'all' and 'notAll' relationships that have the element as a target.  Please recheck submission."),
                    ...
                    if dimTargetRequired and len(hcDimRels) == 0:
                        val.modelXbrl.error(("EFM.6.16.09", "GFM.1.08.09"),
                            _("[fs-1609-Target-Role-With-No-Consecutive-Relationships]  A table is malformed because the $(arcroleURI) from $(fromConcept) to $(toConcept) in link role $(linkroleDefinition) has no consecutive relationships, in $(refSource).  Remove the targetRole attribute, or provide a consecutive relationship."),
                        ...
                        if isinstance(dim, ModelConcept):
                            ...
                            if hasHypercubeArcrole == XbrlConst.all:
                                ...
                                val.modelXbrl.error(("EFM.6.16.07", "GFM.1.08.08"),
                                    _("[fs-1607-Axis-Excluded-Not-In-Table] Members of axis $(dimension) are excluded, in role $(linkroleDefinition), from primary item $(primaryItem) but not included in any table.  Please modify the relationships to make it consistent."),
                            ...
                            if cycleCausingConcept is not None:
                                ...
                                val.modelXbrl.error(("EFM.6.16.04", "GFM.1.08.04"),
                                    _("[fs-1604-Domain-Is-Tangled]  Element $(conceptFrom) appears in overlapping sets of members in a Directed Relationship Set in role $(linkroleDefinition); remove it as a target of one of its domain-member relationships. Please recheck submission."),
                ...
                if hasHypercubeArcrole == XbrlConst.all and len(hasHcRels) > 1:
                    val.modelXbrl.error(("EFM.6.16.05", "GFM.1.08.05"),
                        _("[fs-1605-Primary-Element-Has-Redundant-Tables]  Element $(concept) is a primary element of more than one Table in relationship group $(linkroleDefinition), to tables $(hypercubes) in $(refSources2_n). Please modify the 'all' relationships that have the element as a source so that it has only one primary axis."),
