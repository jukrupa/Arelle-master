    ...
    if modelXbrl.modelDocument.type == ModelDocument.Type.INSTANCE or isInlineXbrl:
                    ...
                    if not entityIdentifierValue:
                        ...
                        if isEFM and not efmCIKpattern.match(entityIdentifierValue):
                            val.modelXbrl.error("EFM.6.05.23.cikValue",
                                _("[cp-0523-Non-Matching-Cik] The context identifier CIK $(entityIdentifier) is not 10 digits, for required context(s).  Please include a correct context identifier CIK in the filing."),
                        ...
                        modelXbrl.error(("EFM.6.05.03", "GFM.1.02.03"),
                            _("[cp-0523-Non-Matching-Cik] The submission CIK, $(filerIdentifier) does not match either the EntityCentralIndexKey, $(entityIdentifer), or context identifier CIK(s) $(entityIdentifer), $(entityIdentifer2), or is not 10 digits, for required context(s).  Please include a correct matching EntityCentralIndexKey and context identifier CIK(s) in the filing."),
        ...
        for context in contexts:
            ...
            if h in uniqueContextHashes:
                if context.isEqualTo(uniqueContextHashes[h]):
                    modelXbrl.error(("EFM.6.05.07", "GFM.1.02.07"),
                        _("[du-0507-Duplicate-Contexts] The instance document contained more than one context equivalent to $(context) ($(context2)).  Please remove duplicate contexts from the instance."),
            ...
            if notAllowed:
                if validateEFMpragmatic:
                    ...
                    modelXbrl.error(("EFM.6.05.04", "GFM.1.02.04"),
                        _("[cp-0504-No-Scenario] There must be no contexts with scenarios, but $(count) was(were) found: $(context)."),
            ...
            if context.isForeverPeriod:
                val.modelXbrl.error("EFM.6.05.38",
                    _("[du-0538-Context-Has-Period-Forever] Context $(contextID)s uses period <xbrli:forever>. Please remove it and resubmit."),
        ...
        for f in modelXbrl.facts:
            ...
            if concept is None:
                modelXbrl.error(("EFM.6.04.03", "GFM.2.01.01"),
                    _("Fact $(fact) of context $(contextID) has an XBRL error"),
                ...
                if concept.isNumeric:
                    if f.precision:
                        modelXbrl.error(("EFM.6.05.17", "GFM.1.02.16"),
                            _("[fs-0517-Decimals-Not-Precision] Your filing contained elements using the precision attribute.  Please recheck your submission and replace the precision attribute with the decimals attribute."),
                ...
                if isEFM and concept.type is not None and concept.type.isDomainItemType:
                    modelXbrl.error("EFM.6.05.25",
                        _("[du-0525-Domain-As-Fact] The domain item $(fact) cannot appear as a fact, in $(refSource).  Please remove the fact from context $(contextID)."),
                        modelObject=f, fact=f.qname, contextID=factContextID)
            ...
            if disclosureSystem.deiFilerIdentifierElement in deiItems:
                ...
                if entityIdentifierValue != value:
                    val.modelXbrl.error(("EFM.6.05.23", "GFM.3.02.02"),
                        _("[cp-0523-Non-Matching-Cik] The EntityCentralIndexKey, $(value), does not match the context identifier CIK $(entityIdentifier).  Please include a correct matching EntityCentralIndexKey and context identifier CIK(s) in the filing."),
                ...
                if val.paramFilerIdentifierNames:
                    if value not in val.paramFilerIdentifierNames:
                        val.modelXbrl.error(("EFM.6.05.23.submissionIdentifier", "GFM.3.02.02"),
                            _("[cp-0523-Non-Matching-Cik] The submission CIK, $(filerIdentifier) does not match the EntityCentralIndexKey.  Please include a correct matching EntityCentralIndexKey in the filing."),
                            modelObject=f, elementName=disclosureSystem.deiFilerIdentifierElement,
                ...
                if val.paramFilerIdentifierNames and entityIdentifierValue in val.paramFilerIdentifierNames:
                    ...
                    if prefix is not None and not value.lower().startswith(prefix.lower()):
                        val.modelXbrl.error(("EFM.6.05.24", "GFM.3.02.02"),
                            _("[cp-0524-Registrant-Name-Mismatch] The Official Registrant name, $(prefix), does not match the value $(value) in the Required Context.  Please correct dei:EntityRegistrantName."),
        ...
        if len(contextIDs) > 0: # check if contextID is on any undefined facts
            ...
            if len(contextIDs) > 0:
                modelXbrl.error(("EFM.6.05.08", "GFM.1.02.08"),
                                _("[du-0508-Unused-Context] The instance document contained a context $(contextIDs) that was not used in any fact. Please remove the context from the instance."),
                        ...
                        if c1 != c2 and c2.isStartEndPeriod:
                            ...
                            if duration > datetime.timedelta(0) and duration <= datetime.timedelta(1):
                                modelXbrl.error(("EFM.6.05.09", "GFM.1.2.9"),
                                    _("[fs-0509-Start-And-End-Dates-Not-Distinct-Inconsistent-With-Document-Type] Context $(endContexts) endDate and $(startContexts) startDate have a duration of one day; that is inconsistent with document type $(documentType)."),
            ...
            if disclosureSystemVersion[0] < 27:
                for end, probCntxs in probInstantCntxsByEnd.items():
                    modelXbrl.error("EFM.6.05.10",
                        _("[fs-0510-Start-And-Instant-Dates-Not-Distinct-Inconsistent-With-Document-Type]  Contexts $(contexts) have an overlap of one day; that is inconsistent with document type $(documentType)."),
                        modelObject=probCntxs, endDate=XmlUtil.dateunionValue(end, subtractOneDay=True), 
        ...
        if val.requiredContext is None:
            modelXbrl.error(("EFM.6.05.19", "GFM.1.02.18"),
                _("[cp-0519-Required-Context] Required context (no segment) not found for document type $(documentType)."),
            ...
            if h in uniqueUnitHashes:
                if unit.isEqualTo(uniqueUnitHashes[h]):
                    modelXbrl.error(("EFM.6.05.11", "GFM.1.02.10"),
                        _("[du-0511-Duplicate-Units] There is more than one unit equivalent to $(unitID) ($(unitID2)).  Please remove all but one and resubmit."),
                    ...
                    if isinstance(measureElt.xValue, ModelValue.QName) and len(measureElt.xValue.localName) > 65:
                        ...
                        if l > 200:
                            modelXbrl.error("EFM.6.05.36",
                                _("[du-0536-name-length-limit] Unit $(unitID) contains a measure element whose local-name in UTF-8 has more than 200 bytes.  Shorten the measure name."),
                        ...
                        try:
                            ...
                            if insignificance: # if not None, returns (truncatedDigits, insiginficantDigits)
                                modelXbrl.error(("EFM.6.05.37", "GFM.1.02.26"),
                                    _("[du-0537-Nonzero-Digits-Truncated] Fact $(fact) of context $(contextID) decimals $(decimals) value $(value) has insignificant digits $(insignificantDigits).  Please correct the fact value and resubmit."),
        ...
        for keyNotDefaultLang, factNotDefaultLang in keysNotDefaultLang.items():
            ...
            if not anyDefaultLangFact:
                val.modelXbrl.error(("EFM.6.05.14", "GFM.1.02.13"),
                    _("[du-0514-English-Text-Missing] Element $(fact) in context $(contextID) has text with xml:lang other than 'en-US' without matching English text.  Please provide a fact with xml:lang equal to 'en-US'."),
        ...
        if amendmentFlag is None:
            modelXbrl.log("WARNING" if validateEFMpragmatic else "ERROR",
                ...
                _("[cp-0521-Amendment-Flag-Existence] You did not include the following data: AmendmentFlag.  Please include AmendmentFlag."),
        ...
        if not documentPeriodEndDate:
            modelXbrl.error(("EFM.6.05.20.missingDocumentPeriodEndDate", "GFM.3.02.01"),
                _("[cp-0520-Document-Period-End-Date-Existence] You did not include the following data: DocumentPeriodEndDate.  Please include DocumentPeriodEndDate."),
        ...
        if isEFM:
            if amendmentFlag == True and amendmentDescription is None:
                modelXbrl.log("WARNING" if validateEFMpragmatic else "ERROR",
                    ...
                    _("[fs-0520-Amendment-Flag-Without-Amendment-Description]  AmendmentFlag is true in context $(contextID), so you must also provide an AmendmentDescription in that context."),
            ...
            if amendmentDescription is not None and amendmentFlag != True:
                modelXbrl.log("WARNING" if validateEFMpragmatic else "ERROR",
                    ...
                    _("[fs-0520-Amendment-Description-Without-Amendment-Flag] AmendmentFlag is not true in context $(contextID), so do not provide an AmendmentDescription in that context."),
            ...
            if documentType is None:
                modelXbrl.error("EFM.6.05.20.missingDocumentType",
                    _("[cp-0520-Document-Type-Existence] You did not include the following data: DocumentType  Please include DocumentType."), 
                    modelXbrl=modelXbrl)
                ...
                modelXbrl.error("EFM.6.05.20.documentTypeValue",
                    _("[cp-0520-Document-Type-Value] Your dei:DocumentType has an illegal value, $(documentType).  Please correct it."),
                ...
                if expectedDocumentTypes and documentType not in expectedDocumentTypes:
                    modelXbrl.error("EFM.6.05.20.submissionDocumentType" if val.paramExhibitType != "EX-2.01" else "EFM.6.23.03",
                        _("[cp-0520-Document-Type-Not-Matched-To-Submission-Type] The value for dei:DocumentType, $(documentType), does not match the submission type, $(submissionType). EDGAR Filer Manual section 6.5.20 lists allowed submission types."),
                        ...
                        if deiItem not in deiItems or deiItems[deiItem] is None: #must exist and value must be non-empty (incl not nil)
                            modelXbrl.log(("WARNING" if validateEFMpragmatic and deiItem in {
                                          ...
                                          ("EFM.6.05.21.{0}".format(deiItem) if validateEFMpragmatic and deiItem in {
                                            ...
                                            _("[cp-0521-Required-Element] Element $(elementName) is required for document type '$(documentType)' in required context $(contextID).  Please include it."),
                    modelObject=documentTypeFact, contextID=documentTypeFact.contextID, documentType=documentType,
                    elementName=deiItem,
                ...
                if commonSharesClassMembers:
                    if defaultContextSharesOutstandingValue is not None: # checks that it exists and is not empty or nil
                        modelXbrl.log(errLevel, "EFM.6.05.26",
                            _("[im-0526-Entity-Common-Shares-Outstanding]  An instance with dei:DocumentType of $(documentType) must have one dei:EntityCommonStockSharesOutstanding fact for each class of stock or units outstanding."),
                            modelObject=documentTypeFact, contextID=documentTypeFact.contextID, documentType=documentType)
            ...
            if documentType in ("SD", "SD/A"): # SD documentType
                        ...
                        if rxdUri.endswith(".xsd") and rxdLoc.elements == "1":
                            if rxdNs is None:
                                ...
                                modelXbrl.error("EFM.6.23.10",
                                    _("[rxd-2310-RXD-Version-Value] The DTS of $(instance) must use only one version of the RXD schema."),
                ...
                if not hasRxdPre:
                    modelXbrl.error("EFM.6.23.08",
                        _("[rxd-2308-RXD-Presentation-Existence] The DTS of $(instance) must use a standard presentation linkbase from Family RXD in edgartaxonomies.xml."),
                        modelObject=modelXbrl, instance=instanceName)
                if not hasRxdDef:
                    modelXbrl.error("EFM.6.23.09",
                        _("[rxd-2309-RXD-Definition-Existence] The DTS of $(instance) must use a standard definition linkbase from Family RXD in edgartaxonomies.xml."),
                        modelObject=modelXbrl, instance=instanceName)
                countryNs = None
                deiNS = None
                        ...
                        if url.startswith("http://xbrl.sec.gov/country/"):
                            if countryNs is None:
                                ...
                                modelXbrl.error("EFM.6.23.11",
                                    _("[rxd-2311-Country-Version-Value] The DTS of $(instance) must use only one version of the COUNTRY schema."),
                                    modelObject=(doc
                                                 for url,doc in modelXbrl.urlDocs.items()
                ...
                if rxdNs:
                    ...
                    if amendmentFlag == True and (
                        ...
                        modelXbrl.error("EFM.6.23.06",
                            _("[rxd-2306-Amendment-Number-Existence] The value for dei:DocumentType, $(documentType), requires a value for rxd:AmendmentNumber in the Required Context."),
                            modelObject=modelXbrl, documentType=documentType)
                else:
                    modelXbrl.error("EFM.6.23.07",
                        _("[rxd-2307-RXD-Schema-Existence] The DTS of $(instance) must use a standard schema from Family SD in edgartaxonomies.xml."),
                        modelObject=modelXbrl, instance=instanceName)
                ...
                if f1 is not None and documentPeriodEndDateFact is not None and f1.xValid >= VALID and documentPeriodEndDateFact.xValid >= VALID:
                    ...
                    if f1.xValue.month != d.month or f1.xValue.day != d.day:
                        modelXbrl.error("EFM.6.23.26",
                            _("[rxd-2326-Fiscal-Year-End-Date-Value] The financial period $(reportingPeriod) does not match the fiscal year end $(fyEndDate)."),
                ...
                if (documentPeriodEndDateFact is not None and documentPeriodEndDateFact.xValid >= VALID and
                    ...
                    modelXbrl.error("EFM.6.23.27",
                        _("[rxd-2327-Payment-Financial-Period-Existence] The financial period $(reportingPeriod) does not match rxd:D in any facts."),
                        ...
                        for concept in XmlUtil.children(doc.xmlRootElement, XbrlConst.xsd, "element"):
                            ...
                            if not concept.isAbstract and not concept.isTextBlock:
                                modelXbrl.error("EFM.6.23.12",
                                    _("[rxd-2312-Custom-Element-Value] $(schemaName) contained a non-abstract declaration for $(name) that is not a Text Block.  Use a standard element or change the type to Text Block."),
                                ...
                                modelXbrl.error("EFM.6.23.13",
                                    _("[rxd-2313-Custom-Dimension-Existence] $(schemaName) contained a declaration for $(name) that is not allowed.  Use dimensions from a standard schema instead."),
                                    modelObject=concept, schemaName=doc.basename, name=concept.name, concept=concept.qname)
                ...
                for rel in dimDomRelSet.modelRelationships:
                    if (isinstance(rel.fromModelObject, ModelConcept) and isinstance(rel.toModelObject, ModelConcept) and 
                        ...
                        modelXbrl.error("EFM.6.23.14",
                            _("[rxd-2314-Dimension-Domain-Relationship-Existence] In $(linkbaseName) the target of the dimension-domain relationship in role $(linkrole) from $(source) to $(target) must be the default member of $(source)."),
                                ...
                                if isinstance(dim, ModelConcept) and isinstance(mem, ModelConcept):
                                    if dim.qname == rxd.PaymentTypeAxis and not mem.modelDocument.targetNamespace.startswith("http://xbrl.sec.gov/rxd/"):
                                        modelXbrl.error("EFM.6.23.17",
                                            _("[rxd-2317-Payment-Type-Dimension-Value] The member $(member) of dimension rxd:PaymentTypeAxis in link role $(linkrole) must be a QName with namespace that begins with 'http://xbrl.sec.gov/rxd/'."),
                                    ...
                                    if dim.qname == rxd.CountryAxis and not mem.modelDocument.targetNamespace.startswith("http://xbrl.sec.gov/country/"):
                                        modelXbrl.error("EFM.6.23.18",
                                            _("[rxd-2318-Country-Dimension-Member-Value] The member $(member) of dimension rxd:CountryAxisAxis in link role $(linkrole) must be a QName with namespace that begins with 'http://xbrl.sec.gov/country/'."),
                                            modelObject=(rel, memRel, dim, mem), member=mem.qname, linkrole=rel.linkrole)
                            ...
                            if rel.consecutiveLinkrole == ELR and isinstance(rel.fromModelObject, ModelConcept):
                                ...
                                if (mem,linkrole) not in memDim:
                                    ...
                                    modelXbrl.error("EFM.6.23.16",
                                        _("[rxd-2316-Member-Multiple-Axis-Existence] Member element $(member) appears in more than one axis: $(dimension1) and $(dimension2).  Use distinct members for the Project, Country, Government, Legal Entity, Business Segment and Payment Type axes."),
                        ...
                        for rel2 in modelXbrl.relationshipSet(XbrlConst.domainMember, rel.consecutiveLinkrole).fromModelObject(rel.toModelObject):
                            if isinstance(rel2.fromModelObject, ModelConcept) and isinstance(rel2.toModelObject, ModelConcept):
                                modelXbrl.error("EFM.6.23.15",
                                    _("[rxd-2315-Consecutive-Domain-Member-Relationships-Existence] The domain-member relationship in $(linkrole)s from $(source)s to $(target)s is consecutive with domain-member relationship in $(linkrole2)s to $(target2)s."),
                                    modelObject=(rel, rel.fromModelObject, rel.toModelObject), 
                ...
                for cntxFacts in cntxEqualFacts.values():
                    ...
                    if (rxd.Co in qnameFacts and not qnameFacts[rxd.Co].isNil and
                        ...
                        modelXbrl.error("EFM.6.23.44",
                            _("[rxd-2344- Country-Value] The value of rxd:Co in context $(context), $(value), is not in the domain of rxd:CountryAxis. Change the value of rxd:Co or add it $(value) to the Country Axis."),
                    ...
                    for f in cntxFacts:
                        if (not context.hasDimension(rxd.PmtAxis) and f.isNumeric and 
                            ...
                            modelXbrl.error("EFM.6.23.37",
                                _("[rxd-2337-Payment-Unit-Value] An $(fact) fact in context $(context) has unit $(unit) not matching the dei:EntityReportingCurrencyISOCode $(currency)."),
                    ...
                    if (rxd.A in qnameFacts and not qnameFacts[rxd.A].isNil and
                        ...
                        modelXbrl.error("EFM.6.23.38",
                            _("[rxd-2338-Conversion-Method-Value] A value cannot be given for rxd:Cm in context $(context) because the payment is in the reporting currency $(currency)."),
                    ...
                    if (rxd.A in qnameFacts and 
                        ...
                        modelXbrl.error("EFM.6.23.41",
                            _("[rxd-2341-Amount-Currency-Existence] The unit $(unit) of rxd:A in context $(context) is not consistent with the value $(currency) of rxd:Cu.  Change one or the other to match."),
                    ...
                    if (context.hasDimension(rxd.ProjectAxis) and
                        ...
                        modelXbrl.error("EFM.6.23.19",
                            _("[rxd-2319-Project-Payment-Amount-Existence] A payment for each project axis member is required.  Provide a value for element rxd:Pr with value $(dimension) in context $(context)."),
                    ...
                    if (context.hasDimension(rxd.GovernmentAxis) and
                        ...
                        modelXbrl.error("EFM.6.23.21",
                            _("[rxd-2321-Government-Payment-Amount-Existence] A payment amount for each government is required.  Provide a value for element rxd:Gv with value $(member)."),
                    ...
                    if rxd.P in qnameFacts and not any(f.context is not None and not f.context.hasSegment
                        ...
                        modelXbrl.error("EFM.6.23.23",
                            _("[rxd-2323-Category-Total-Existence] Payment type $(paymentType) was reported in context $(context) but there is no fact with element $(paymentType) in the Required Context."),
                    ...
                    if not context.hasDimension(rxd.PmtAxis) and rxd.A in qnameFacts and not qnameFacts[rxd.A].isNil:
                        modelXbrl.error("EFM.6.23.40",
                            _("[rxd-2340-Amount-Line-Existence] There is a fact with element rxd:A in context $(context) not defining its payment number."),
                ...
                for f in modelXbrl.factsByQname[rxd.D]:
                    if not f.isNil and f.xValid >= VALID and f.xValue + datetime.timedelta(1) != f.context.endDatetime: # date needs to be midnite to compare to datetime
                        modelXbrl.error("EFM.6.23.32",
                            _("[rxd-2332-Date-Value] The value of rxd:D in context $(context), $(value), is not equal to $(endDate).  Change the context end date or the value."),
                        ...
                        if (not isinstance(child,ModelInlineFootnote) and
                            (child.namespaceURI != XbrlConst.link or 
                             ...
                             child.localName not in ("loc", "footnote", "footnoteArc"))):
                                modelXbrl.error(("EFM.6.05.27", "GFM.1.02.19"),
                                    _("[du-0527-Footnote-Substitution-Group]  Footnote link $(footnoteLinkNumber) has a child element $(elementName) that is not allowed.  Please remove it."),
                            ...
                            if locrole is not None and (disclosureSystem.GFM or \
                                ...
                                modelXbrl.error(("EFM.6.05.29", "GFM.1.02.21"),
                                    _("[du-0529-Footnote-Custom-Loc-Role] Footnote locator $(xlinkLabel) has a custom role, $(role) that is not allowed.  Please replace it with the default footnote role."),
                            ...
                            if not href.startswith("#"): 
                                modelXbrl.error(("EFM.6.05.32", "GFM.1.02.23"),
                                    _("[du-0532-Footnote-Locator-Portable]  Footnote $(locLabel) refers to a location, $(locHref), that does not begin with '#' so that any change to the file name would render the XBRL invalid."),
                            ...
                            if (isEFM and not disclosureSystem.uriAuthorityValid(arcrole)) or \
                               (disclosureSystem.GFM  and arcrole != XbrlConst.factFootnote and arcrole != XbrlConst.factExplanatoryFact): 
                                modelXbrl.error(("EFM.6.05.30", "GFM.1.02.22"),
                                    _("[du-0530-Footnote-Custom-Arcrole] Footnote relationship $(arcToLabel) has a custom arc role $(arcrole) that is not allowed.  Please replace it with the default (fact-footnote) arcrole."),
                            ...
                            if footnoterole == "":
                                modelXbrl.error(("EFM.6.05.28.missingRole", "GFM.1.2.20"),
                                    _("[du-0528-Footnote-Custom-Footnote-Role] Footnote $(xlinkLabel) is missing a role. Please provide the default footnote role."),
                                ...
                                modelXbrl.error(("EFM.6.05.28", "GFM.1.2.20"),
                                    _("[du-0528-Footnote-Custom-Footnote-Role]  Footnote $(xlinkLabel) has a role $(role) that is not allowed. Please replace it with the default footnote role."),
                                    modelObject=child, xlinkLabel=getattr(child, "xlinkLabel", None),
                                    role=footnoterole)
                            ...
                            if XmlUtil.text(child) != "" and not isInlineXbrl:
                                ...
                                if not foundFact:
                                    modelXbrl.error(("EFM.6.05.33", "GFM.1.02.24"),
                                        _("[cp-0533-Dangling-Footnote] The footnote with label $(footnoteLabel) and text '$(text)' is not connected to any fact.  Please remove the footnote, or link it to a fact."),
                                        modelObject=child, footnoteLinkNumber=footnoteLinkNbr, 
            ...
            for _prefix, _ns in elt.nsmap.items():
                if _ns == ns and _prefix != prefix:
                    modelXbrl.error("EFM.5.02.05.standardNamespacePrefix",
                        _("[ix-0502-Standard-Namespace-Prefix] The prefix $(submittedPrefix) must be replaced by $(recommendedPrefix) for standard namespace $(namespace)"),
            ...
            if hiddenFactRefMatch:
                ...
                if hiddenFactRef not in hiddenEltIds:
                    modelXbrl.error("EFM.5.02.05.14.hidden-fact-not-found",
                        _("[ix-0514-Hidden-Fact-Not-Found] The value of the -sec-ix-hidden style property, $(id), does not correspond to the id of any hidden fact."),
                ...
                if role == XbrlConst.documentationLabel:
                    if concept.modelDocument.targetNamespace in disclosureSystem.standardTaxonomiesDict:
                        modelXbrl.error(("EFM.6.10.05", "GFM.1.05.05"),
                            _("[cp-1005-Custom-Documentation-Standard-Element] Your filing attempted to add a new definition, '$(text)', to an existing concept in the standard taxonomy, $(concept).  Please remove this definition."),
                    ...
                    if role == XbrlConst.standardLabel:
                        if text in defaultLangStandardLabels:
                            ...
                            modelXbrl.error(("EFM.6.10.04", "GFM.1.05.04"),
                                _("[du-1004-English-Standard-Labels-Duplicated] More than one element has $(text) as its English standard label ($(concept) in $(refSource) and $(concept2) in $(refSource3)).  Please change or remove all but one label."),
                    ...
                    if len(text) > 511:
                        modelXbrl.error(("EFM.6.10.06", "GFM.1.05.06"),
                            _("[rq-1006-Label-Disallowed]  Element $(concept) has more than 511 characters or contains a left-angle-bracket character in the label for role $(role), in $(refSource). Please correct the label."),
                ...
                if (text is not None and len(text) > 0 and 
                   ...
                   (modelXbrl.modelManager.disclosureSystem.labelTrimPattern.match(text[0]) or \
                    ...
                    modelXbrl.error(("EFM.6.10.08", "GFM.1.05.08"),
                        _("[du-1008-Label-Not-Trimmed] The label $(text) of element $(concept) has leading or trailing white space, in $(refSource).  Please remove it."),
            ...
            if modelRefRel.modelDocument.inDTS: # ignore references added by EdgarRenderer that are not in DTS
                ...
                if concept.modelDocument.targetNamespace not in disclosureSystem.standardTaxonomiesDict:
                    modelXbrl.error(("EFM.6.18.01", "GFM.1.9.1"),
                        _("[cp-1801-Custom-Element-Has-Reference] Your filing provides a reference, '$(xml)', for an custom concept in extension taxonomy, $(concept).  Please remove this reference."),
                    ...
                    modelXbrl.error(("EFM.6.18.02"),
                        _("[cp-1802-Standard-Element-Has-Reference] Your filing attempted to add a new reference, '$(xml)', to an existing concept in the standard taxonomy, $(concept).  Please remove this reference."),
                        modelObject=modelReference, concept=concept.qname, text=text, xml=XmlUtil.xmlstring(modelReference, stripXmlns=True, contentsOnly=True))
    ...
    for roleURI, modelRoleTypes in modelXbrl.roleTypes.items():
        ...
        if countInDTS > 1:
            modelXbrl.error(("EFM.6.07.10", "GFM.1.03.10"),
                _("[du-0710-Role-Type-Duplicates]  Role $(roleType) was declared more than once ($(numberOfDeclarations) times, in $(refSources)).  Please remove all but one declaration."),
    ...
    for arcroleURI, modelRoleTypes in modelXbrl.arcroleTypes.items():
        ...
        if countInDTS > 1:
            modelXbrl.error(("EFM.6.07.14", "GFM.1.03.16"),
                _("[du-0714-Arcrole-Type-Duplicates] Relationship arc role $(arcroleType) is declared more than once ($(numberOfDeclarations) duplicates, $(refSources)).  Please remove all but one of them."),
        ...
        for conflictClass, modelDocuments in val.standardNamespaceConflicts.items():
            if len(modelDocuments) > 1:
                modelXbrl.error("EFM.6.22.03.incompatibleSchemas",
                    _("[cp-2203-Incompatible-Taxonomy-Versions]  References for conflicting standard taxonomies $(conflictClass) are not allowed in same DTS $(modelObject)"),
                ...
                for modelRel in ineffectiveArcs:
                    if isinstance(modelRel.fromModelObject, ModelObject) and isinstance(modelRel.toModelObject, ModelObject):
                        modelXbrl.error(("EFM.6.09.03", "GFM.1.04.03"),
                            _("[du-0903-Relationship-Ineffectual] The $(arcrole) relationship from $(conceptFrom) to $(conceptTo), link role $(linkroleDefinition), in the submission is ineffectual.  Please remove or correct the relationship."),
                        ...
                        for rel in siblingRels:
                            ...
                            if relTo in conceptsUsed:
                                ...
                                if preferredLabel in preferredLabels:
                                    ...
                                    modelXbrl.error(("EFM.6.12.05", "GFM.1.06.05"),
                                        _("[rq-1205-Preferred-Label-Duplicates] Relationships from $(fromConcept) to $(concept) in role $(linkroleDefinition) do not have distinct values for the preferredLabel attribute, in $(refSources).  Change all but one value of preferredLabel."),
                            ...
                            if order in orderRels:
                                modelXbrl.error(("EFM.6.12.02", "GFM.1.06.02"),
                                    _("[rq-1202-Presentation-Order-Duplicates] More than one presentation relationship in role $(linkroleDefinition) has order value $(order), in $(refSources).  Change all but one so they are distinct."),
                    ...
                    for relFrom, rels in fromRelationships.items():
                        ...
                        if directedCycleRels is not None:
                            val.modelXbrl.error(("EFM.6.14.04", "GFM.1.07.04"),
                                _("[fs-1404-Circular-Calculation] Element $(concept) is summed into itself in calculation group $(linkroleDefinition).  Please recheck submission."),
                        ...
                        for rel in rels:
                            ...
                            if not (isinstance(relTo, ModelConcept) and relTo.type is not None and relTo.type.isDomainItemType) and not isStandardUri(val, rel.modelDocument.uri):
                                val.modelXbrl.error(("EFM.6.16.03", "GFM.1.08.03"),
                                    _("[du-1603-Dimension-Domain-Target-Mismatch] There is a dimension-domain relationship from $(conceptFrom) but its target element $(conceptTo) is not a domain, in $(refSource).  Please change the relationship or change the type of the target element."),
    ...
    for concept, hasPresentationRelationship in conceptsUsed.items():
        if not hasPresentationRelationship:
            val.modelXbrl.error(("EFM.6.12.03", "GFM.1.6.3"),
                _("[cp-1203-Element-Used-Not-Presented] Element $(concept) is used in a fact or context in the instance, but is not in any presentation relationships.  Element is defined in $(refSources2_n).  Add the element to at least one presentation group."),
    ...
    for fromIndx, toIndxs in usedCalcsPresented.items():
        for toIndx in toIndxs:
            ...
            val.modelXbrl.error(("EFM.6.14.05", "GFM.1.7.5"),
                _("[du-1405-Facts-In-Calculations-Presentation-Missing] Context $(contextId) has facts of elements $(conceptFrom) and $(conceptTo), with a calculation relationship in $(linkroleDefinition), but these elements are not in any common corresponding presentation relationship group.  The calculation relationship is in $(refSource)."),
...
def checkConceptLabels(val, modelXbrl, labelsRelationshipSet, disclosureSystem, concept):
        ...
        if isinstance(modelLabel, ModelResource) and modelLabel.xmlLang and modelLabel.modelDocument.inDTS:
            ...
            if dupDetectKey in dupLabels:
                modelXbrl.error(("EFM.6.10.02", "GFM.1.5.2"),
                    _("Concept $(concept) has duplicated labels for role $(role) lang $(lang)."),
    ...
    if not hasDefaultLangStandardLabel:
        modelXbrl.error(("EFM.6.10.01", "GFM.1.05.01"),
            _("[cp-1001-Element-Used-Standard-Label] You have submitted an instance using an element without an en-US standard label $(concept) in $(refSources2_n). Please check your submission and correct the labels."),
            ...
            if role != priorRole:
                if not hasDefaultLang:
                    modelXbrl.error(("EFM.6.10.03", "GFM.1.5.3"),
                        _("[cp-1003-Element-Used-Standard-English-Label] You have submitted an instance using an element $(concept) with $(lang) for role $(role) in instance $(refSource). Labels are in $(refSources2_n). Please check your submission and correct the labels."),
